function [S_STRUCT, CP1, CM1] = J_UNIWARD_matlab(coverPath, payload)
...
% 1D high pass decomposition filter(hpdf)
...
% 1D low pass decomposition filter(lpdf)
...
for row = 1:k
    for col = 1:l
        ...
        %rhoTemp = tempXi{1} + tempXi{2} + tempXi{3} + tempXi{4}; %%%%%%%%%%%%%%%
        %rhoTemp = tempXi{3};
...
[S_COEFFS, CP1, CM1] = EmbeddingSimulator(C_COEFFS, rhoP1, rhoM1, round(payload * nzAC));
% Beta = (CP1 + CM1)/2;
